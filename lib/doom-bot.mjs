...
export function createBot(engine) {
  ...
  function isInGame() {
    ...
    if (variance < HUD_GRAY_VARIANCE_THRESH) {
      // Low variance alone is not enough — a FLAT RED strip (title art,
      // lava floor) also has near-zero variance. The HUD is gray: channel
      // means must sit close together.
      const spread = Math.max(
        Math.abs(meanR - meanG),
        Math.abs(meanG - meanB),
        Math.abs(meanR - meanB),
      );
      if (spread < 25) return true; // uniform GRAY strip → in-game HUD
    }
  ...
  function runStartSequence(nowMs) {
    ...
    // Later taps abort if the game started meanwhile — a transient isInGame()
    // misread (bright red rooms push redFraction over the threshold) must not
    // fire blind ENTERs into live gameplay and restart it via the menu.
    tap(KEY_ENTER, nowMs);
