...
export function createBot(engine) {
  ...
  function update(nowMs, aggressive, order = null) {
    ...
    // ── Targeting: motion first, flesh second (cerebellum) ──────────────────
    // Enemies and projectiles MOVE; lava, torches and red walls don't.
    // Firing requires motion evidence — fleshy pixels alone only count when
    // the scene shows corroborating motion.
    ...
    const monsterSeen =
      motion.center > MOTION_FIRE_THRESH ||
      (ratio > threshold && (motion.left + motion.center + motion.right) > MOTION_CORROBORATE);

    // Aim assist: clearly hotter side zone → short turn toward it
    if (!monsterSeen && !_stuckTurning && !_wanderTurnKey) {
      const sideMax = Math.max(motion.left, motion.right);
      if (sideMax > MOTION_AIM_THRESH && sideMax > motion.center * 1.5) {
        const turnKey = motion.left > motion.right ? KEY_LEFTARROW : KEY_RIGHTARROW;
        engine.pushKey(1, turnKey);
        _wanderTurnKey = turnKey;
